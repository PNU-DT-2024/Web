...
export default function IndivSelect({ type }) {
  ...
  const handleMouseEnter = (name) => {
    ...
    // 애니메이션 리셋
    const imgTrigger = document.querySelector(`.${styles.imgTrigger}`);
    if (imgTrigger) {
      imgTrigger.classList.remove(styles.show);
      void imgTrigger.offsetWidth; // 애니메이션 재시작
      imgTrigger.classList.add(styles.show);
    }
  };
      ...
      <article className={`${styles.contentList} column`}>
        {indivData.list.map((item, index) => (
          <Link
            key={item.index}
            to={`indiv/${item.name}`}
            className={styles.selectIndiv}
          >
            <div
              className={`${
                index === indivData.list.length - 1 ? styles.lastList : ""
              } row`}
          ...
          {isName != null ? (
            type === "project" ? (
              <div className={styles.imgThumb}>
                <img
                  src={`/asset/poster/cover/${isName}_cover.webp`}
                  alt={isName}
                  loading="lazy"
                />
              </div>
              ...
              <div className={styles.imgAction}>
                <img
                  src="/asset/profile/trigger.png"
                  alt=""
                  loading="lazy"
                  className={`${styles.imgTrigger} ${isHovered && styles.show}`}
                />
                <img
                  src={
                    isHovered
                      ? `/asset/profile/SC/${isName}_profileSC.webp`
                      : ""
                  }
                  loading="lazy"
                  alt={isName}
            ...
            <div className={styles.imgThumb}>
              <img
                src={`/asset/poster/cover/defalut_cover.webp`}
                alt={isName}
                loading="lazy"
              />
            </div>
            ...
            <div className={styles.imgThumb}>
              <img
                src="/asset/profile/SC/profileBase.webp"
